...
<body>
	<%
		String serviceCategoryNameParam = request.getParameter("name");
		String pageTitle = serviceCategoryNameParam + "&ensp;|&ensp;Services";
		String descriptionContent = "";
		boolean serviceFound = false;
		for(ServiceCategoryDto serviceCategoryData : ServiceCategoryManager.getAllServiceCategories()) {
			String name = serviceCategoryData.name();
			if(serviceCategoryNameParam.equals(name)){
				serviceFound = true;
				descriptionContent = serviceCategoryData.description();
				break;
			}
		}
            ...
            <div class="col-12 col-sm-10">
				...
				if (serviceFound) {
				    for (int i = startIndex; i < endIndex; i++) {
				        ...
				        String imgUrl = "../img/service-" + service.imgIndex() + "-banner.png";
				        String redirectUrl = "./service.jsp?name=" + service.name();
			    ...
			    <nav>
			        <ul class="pagination justify-content-center">
			            ...
			            <li class="page-item <%= (currentPage == 1 ? "disabled" : "") %>">
			                <a class="page-link" href="?name=<%=serviceCategoryNameParam%>&page=<%=currentPage-1%>">Previous</a>
			            </li>
			
			            <% for(int i = 1; i <= totalPages; i++) { %>
			                <li class="page-item <%= (currentPage == i ? "active" : "") %>">
			                    <a class="page-link" href="?name=<%=serviceCategoryNameParam%>&page=<%=i%>"><%=i%></a>
			                </li>
			            <% } %>
			
			            <li class="page-item <%= (currentPage == totalPages ? "disabled" : "") %>">
			                <a class="page-link" href="?name=<%=serviceCategoryNameParam%>&page=<%=currentPage+1%>">Next</a>
			            </li>
